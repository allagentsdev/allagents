...
This matters most when something else populates the directory. A CI job that checks out a pinned commit before calling `allagents update` should leave `managed` unset — or pass `--no-managed` to that run — so AllAgents cannot move the checkout to upstream HEAD. Pulls are skipped rather than forced when the working tree is dirty or is not on the declared `branch`.

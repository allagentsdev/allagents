...
```bash
allagents update [--offline] [--dry-run] [--force] [--verbose] [--no-managed] [--profile <name>...]
allagents status
```
...
| `-f, --force` | Overwrite existing MCP server entries that differ from plugin config |
| `-v, --verbose` | Show informational sync messages |
| `--no-managed` | Skip managed repository clone/pull operations |
...
ordinary user and project sync.

Repositories declared with [`managed`](/docs/reference/configuration/#managed-repositories)
are cloned or pulled before plugin sync. Pass `--no-managed` to skip that step
for a single run — for example in CI, where another tool has already materialized
the repository directories and a stray `managed` entry must not move them.
`--offline` and `--dry-run` skip managed operations too; `--dry-run` does not
preview them.
...
allagents plugin update [plugin] [--scope <scope>]
allagents plugin marketplace add <source> [--name <name>] [--branch <branch>] [--scope <scope>]
...
Update one declared plugin or every plugin in the selected scope.
...
|------|-------------|
| `[plugin]` | Update one declared plugin; omit it to update all selected-scope plugins. |
| `-s, --scope <scope>` | Installation scope: `project` (default), `user`, or `all`. |

When the direct command writes to a TTY, it first names each source as it is
checked without mutating it, reports how many plugin updates were found, then
appends one `Updating ...` line for each source as it applies the update and
leaves that line and the settled result visible. A source whose backing
checkout already matches its remote is reported as skipped and is not
re-applied; the existing client synchronization for its scope still runs.
Internal client synchronization does not introduce a separate status phase.
JSON and redirected human output stay one-shot: JSON remains one document, and
human results remain batched until the update finishes.
Successful client synchronization is folded into the source result; warnings
and failures still include their diagnostic detail.

The interactive TUI's **Plugins → Update all** action uses a different display:
the current plugin or standalone skill source replaces the message on its
existing spinner. After the spinner ends, the existing **Update Results** note
shows the final counts.
...
When the direct command writes to a TTY, it first appends
`Checking skills from source: ...` for each physical source. After preflight,
it reports how many installed skill updates were found, then prints
`Updating <skill>…` and the settled result for each skill that enters apply
work. Sources retained or rejected before mutation never show an update-start
line. JSON and redirected human output stay one-shot: JSON remains one
document, and human results remain batched until the update finishes.
...
Server and tool names are exact protocol data. If either name begins with `-`,
...
allagents mcp call --help --profile markets -- --trading -quote

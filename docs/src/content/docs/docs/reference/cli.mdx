...
When the direct command writes to a TTY, it appends one `Updating ...` line for
each physical source and leaves that line and the settled source result visible.
Internal preflight and client synchronization do not introduce separate status
phases. JSON and redirected human output stay one-shot: JSON remains one

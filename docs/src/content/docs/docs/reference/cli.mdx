...
When the direct command writes to a TTY, it appends one `Updating ...` line for
each source and leaves that line and the settled result visible. Internal
preflight and client synchronization do not introduce separate status phases.

...
When the direct command writes to a TTY, it appends a source line before each
plugin starts and leaves that line and the settled result visible. JSON and
redirected human output stay one-shot: JSON remains one document, and human
results remain batched until the update finishes.
...
When the direct command writes to a TTY, it appends a
`Checking skills from source: ...` line before each physical source check and
leaves that line and the settled source result visible. JSON and redirected
human output stay one-shot: JSON remains one document, and human results remain
batched until the update finishes.

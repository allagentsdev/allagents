...
AllAgents curates the pinned `skills@1.7.0` registry to coding clients that
directly consume skills; non-coding platforms are intentionally excluded. This
compatibility does not rewrite established AllAgents paths. Aliases use their
canonical AllAgents product's existing destinations, while newly added
canonical clients use the pinned Skills paths.

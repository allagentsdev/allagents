...
repositories:
  ...
  - path: ../pinned-lib
    ...
    branch: release-1.x    # Pull only when this branch is checked out
    managed: "clone"       # Clone when missing, never pull
  - path: ../vendored      # No managed field: AllAgents never clones or pulls
```

Pulling never discards work. AllAgents skips the pull and reports a warning when the working tree has uncommitted changes, or when `branch` is set and the checkout is on a different branch.

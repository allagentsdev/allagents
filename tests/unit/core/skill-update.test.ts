  ...
  it('reports deferred preflight starts and typed execution results in sequential unit order', async () => {
    ...
    const installations = [
      ...
      installation({
        id: 'project:1',
        configIndex: 1,
        rawSource: 'acme/tools',
        effectiveSource: 'acme/tools',
        pluginName: 'tools',
        rootNodeId: secondNode.id,
        nodes: [secondNode],
        skills: [{ name: 'tool', subpath: 'tool', enabled: true }],
      }),
    ];
    const events: string[] = [];
        ...
        installations: [
          {
            ...
            outcome: 'resolved',
            skills: entry.skills.map(({ name, subpath }) => ({
              name,
              subpath,
            })),
          },
    ...
    const preflight = buildSkillUpdatePreflight(
      { installations, selectedScopes: ['project'] },
      {
        onUnitStart: (unit) => {
          events.push(`start:${unit.id}`);
          if (unit.id === secondNode.id) secondPreflightStarted.resolve();
        },
        inspectUnit: (unit) => {
          events.push(`inspect:${unit.id}`);
          return unit.id === projectNode.id
    ...
    expect(events).toEqual([
      `start:${projectNode.id}`,
      `inspect:${projectNode.id}`,
    ]);
    ...
    expect(events).toEqual([
      ...
      `inspect:${projectNode.id}`,
      `start:${secondNode.id}`,
      `inspect:${secondNode.id}`,
    ]);
    secondInspection.resolve(
      inspectionFor({ nodes: [secondNode], installations: [installations[1]!] }),
    );
    ...
    const execution = executeSkillUpdatePlan(plan, {}, {
      reconcileUnit: async (unit) => {
        ...
        const first = unit.id === projectNode.id;
    ...
    expect(events.slice(4)).toEqual([`execute:${projectNode.id}`]);
    releaseFirstExecution.resolve();
    await secondExecutionStarted.promise;
    expect(events.slice(4)).toEqual([
      `execute:${projectNode.id}`,
      `result:${projectNode.id}:updated`,
      `execute:${secondNode.id}`,
    ]);
